...
| `Type` | `NVARCHAR(40) NOT NULL` | one of: `card.created`, `card.renamed`, `card.described`, `label.added`, `label.removed`, `member.assigned`, `member.unassigned`, `due.set`, `due.cleared`, `due.completed`, `checklist.item.added`, `checklist.item.checked`, `checklist.item.unchecked`, `checklist.item.deleted`, `comment.added` (FUNCTIONAL_SPEC §5.2, plus `checklist.item.deleted`/`checklist.item.unchecked`, which this feature's C-09 needs and §5.2 didn't originally enumerate — found during Phase A's backend-compliance self-review: unchecking an item is a state change same as checking one, and the append-only feed must record it too) |
