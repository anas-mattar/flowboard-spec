...
- [ ] T001 Add `public byte[] RowVersion { get; set; } = [];` to
  ...
  data-model.md)
- [ ] T002 Generate and apply the EF Core migration (`AddBoardListRowVersion`) in
...
- [ ] T003 [US1] `BoardContentService.CreateBoardAsync` — resolve the caller's own
...
- [ ] T004 [US1] `POST /v1/boards` in
  flowboard-api/src/Flowboard.Api/Endpoints/BoardsEndpoints.cs (depends on T003)
- [ ] T005 [P] [US1] Integration test: creating a board returns the three starter lists
...
- [ ] T006 [US2] `ListService.CreateListAsync` — `CanMutate` (`BoardAdmin`/`BoardMember`);
...
- [ ] T007 [US2] `POST /v1/boards/{boardPublicId}/lists` in
  flowboard-api/src/Flowboard.Api/Endpoints/BoardsEndpoints.cs (depends on T006)
- [ ] T008 [P] [US2] Integration test: a new list lands rightmost among existing lists,
...
- [ ] T009 [US3] `BoardContentService.UpdateBoardAsync` — `access.Role !=
...
- [ ] T010 [US3] `PATCH /v1/boards/{boardPublicId}` in
  flowboard-api/src/Flowboard.Api/Endpoints/BoardsEndpoints.cs (depends on T009)
- [ ] T011 [P] [US3] Integration test: a `BoardAdmin` renames successfully and gets back
...
- [ ] T012 [US4] [US6] `ListService.UpdateListAsync` — accepts optional `name` and/or
...
- [ ] T013 [US4] [US6] `PATCH /v1/lists/{listPublicId}` in
  flowboard-api/src/Flowboard.Api/Endpoints/ListsEndpoints.cs (depends on T012)
- [ ] T014 [P] [US4] [US6] Integration test: renaming persists and returns the new
...
- [ ] T015 [US5] `BoardContentService.StarBoardAsync`/`UnstarBoardAsync` — `CanMutate`;
...
- [ ] T016 [US5] `POST /v1/boards/{boardPublicId}/star` and `.../unstar` in
  flowboard-api/src/Flowboard.Api/Endpoints/BoardsEndpoints.cs (depends on T015)
- [ ] T017 [P] [US5] Integration test: starring then re-fetching `boards.list` (already
...
- [ ] T018 [US7] `BoardContentService.DeleteBoardAsync` — `access.Role !=
...
- [ ] T019 [US7] `DELETE /v1/boards/{boardPublicId}` in
  flowboard-api/src/Flowboard.Api/Endpoints/BoardsEndpoints.cs (depends on T018)
- [ ] T020 [P] [US7] Integration test: deleting removes the board from
...
- [ ] T025 [US9] `ListService.SortByDueDateAsync` — `CanMutate`; order by `CASE WHEN
  ...
  flowboard-api/src/Flowboard.Api/Services/ListService.cs
- [ ] T026 [US9] `POST /v1/lists/{listPublicId}/sort` in
  flowboard-api/src/Flowboard.Api/Endpoints/ListsEndpoints.cs (depends on T025)
- [ ] T027 [P] [US9] Integration test: a list with dated and undated cards sorts
...
- [ ] T028 [US1] Add `createBoardInputSchema` (`name`) to
  flowboard-web/src/lib/boards/schemas.ts
- [ ] T029 [US1] Extend the server-only boards client with `createBoard` in
  flowboard-web/src/lib/api/boards-client.ts (cite
  contracts/board-list-management-api.md; depends on T028)
- [ ] T030 [US1] Extend `boardsRouter` with a `create` `protectedProcedure` in
  flowboard-web/src/server/api/routers/boards.ts (depends on T029)
- [ ] T031 [US1] Add a "+ Create board" inline composer row to
...
- [ ] T032 [US2] Add `createListInputSchema` (`boardPublicId`, `name`) to
  flowboard-web/src/lib/lists/schemas.ts
- [ ] T033 [US2] Extend the server-only lists client with `createList` in
  flowboard-web/src/lib/api/lists-client.ts (depends on T032)
- [ ] T034 [US2] Extend `listsRouter` with a `create` `protectedProcedure` in
  flowboard-web/src/server/api/routers/lists.ts (depends on T033)
- [ ] T035 [US2] Create `add-list-composer.tsx` (mirrors 004's card composer, research
...
- [ ] T036 [US3] Add `renameBoardInputSchema` (`name`) to
  flowboard-web/src/lib/boards/schemas.ts
- [ ] T037 [US3] Extend the boards client with `renameBoard` (sends `If-Match`, maps
  `409`) in flowboard-web/src/lib/api/boards-client.ts (depends on T036)
- [ ] T038 [US3] Extend `boardsRouter` with a `rename` `protectedProcedure` surfacing a
  `CONFLICT` `TRPCError` on `409` and `FORBIDDEN` on `403` in
  flowboard-web/src/server/api/routers/boards.ts (depends on T037)
- [ ] T039 [US3] Create `board-title-bar.tsx` (client component) with an inline-editable
...
- [ ] T040 [US4] Add `updateListInputSchema` (`name?`, `wipLimit?`, at least one
  required) to flowboard-web/src/lib/lists/schemas.ts — the one schema US4 and US6 both
  use
- [ ] T041 [US4] Extend the lists client with `updateList` (sends `If-Match`, maps `409`)
  in flowboard-web/src/lib/api/lists-client.ts — shared by US4 and US6 (depends on T040)
- [ ] T042 [US4] Extend `listsRouter` with an `update` `protectedProcedure` in
  flowboard-web/src/server/api/routers/lists.ts (depends on T041)
- [ ] T043 [US4] Make `list-column.tsx`'s header title inline-editable, calling
...
- [ ] T044 [US5] Extend the boards client with `starBoard`/`unstarBoard` in
  flowboard-web/src/lib/api/boards-client.ts
- [ ] T045 [US5] Extend `boardsRouter` with `star`/`unstar` `protectedProcedure`s in
  flowboard-web/src/server/api/routers/boards.ts (depends on T044)
- [ ] T046 [US5] Wire `board-title-bar.tsx`'s Star button to `boards.star`/`boards.unstar`
...
- [ ] T047 [US6] Create `list-actions-menu.tsx` (client popover) with its "Set WIP limit"
...
- [ ] T048 [US7] Extend the boards client with `deleteBoard` in
  flowboard-web/src/lib/api/boards-client.ts
- [ ] T049 [US7] Extend `boardsRouter` with a `delete` `protectedProcedure` surfacing
  `FORBIDDEN` on `403` in flowboard-web/src/server/api/routers/boards.ts (depends on T048)
- [ ] T050 [US7] Add a `BoardAdmin`-only "Delete board" control to `board-title-bar.tsx`,
...
- [ ] T051 [US8] Extend the lists client with `archiveListCards`/`deleteList` in
  flowboard-web/src/lib/api/lists-client.ts
- [ ] T052 [US8] Extend `listsRouter` with `archiveCards`/`delete` `protectedProcedure`s
  in flowboard-web/src/server/api/routers/lists.ts (depends on T051)
- [ ] T053 [US8] Add "Archive all cards" and "Delete list" rows to
...
- [ ] T054 [US9] Extend the lists client with `sortListByDueDate` in
  flowboard-web/src/lib/api/lists-client.ts
- [ ] T055 [US9] Extend `listsRouter` with a `sort` `protectedProcedure` in
  flowboard-web/src/server/api/routers/lists.ts (depends on T054)
- [ ] T056 [US9] Add a "Sort by due date" row to `list-actions-menu.tsx`, invalidating
...
- [ ] T057 Capture the implemented sidebar (default state), the list "⋯" menu open, and a
...
- [ ] T058 [P] Walk `quickstart.md` end-to-end (US1–US9 rows, all edge cases including
