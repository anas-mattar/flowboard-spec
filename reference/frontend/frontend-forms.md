...
2. **Page-owned mutation**: the page owning the screen passes an async
   ...
   modals/forms over the same query cache (e.g. chart of accounts).
...
| Domain dropdown | `<SupplierDropdown value={field.value} onChange={field.onChange} />` | `@/components/dropdowns/*` |
...
- Never use `.Result` / blocking sync calls in form handlers.
