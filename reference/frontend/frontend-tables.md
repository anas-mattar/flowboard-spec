...
`fms-frontend` uses a **two-layer base table system**. All new tables MUST compose from these base components -- do not duplicate pagination, search, sort, or URL-state logic.
...
- **Matrix-style grids** (dynamic columns, full dataset rendered, no server
  paging — e.g. permission matrix, closing checklist) may compose `BaseDataTable`
  (Layer 1) directly with a `ColumnDef` factory. They still get `columns.tsx` and
  the feature folder; they skip URL state because there is nothing to page.

Matrix-grid support on `BaseDataTable`:

- Per-column styling via tanstack `ColumnDef.meta`: `headerClassName` /
  `cellClassName` are merged onto the `<th>`/`<td>` (e.g. the violet Super User
  column, sticky first columns, centered checkbox cells).
- Labelled section bands via the optional `rowGroupHeader` prop
  ...
  row is rendered whenever `getGroupKey` changes between consecutive rows (e.g.
  the closing checklist's five item groups).
...
Layer 3: Feature tables                   PurchaseOrderTable, SupplierTable, ...
